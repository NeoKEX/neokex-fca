...
async function get(url, reqJar, qs, options, ctx, customHeader) {
    if (ctx && ctx.antiDetectionManager) {
        await ctx.antiDetectionManager.applyRequestDelay();
    }
    
    let headers = getHeaders(url, options, ctx, customHeader);
    if (ctx && ctx.antiDetectionManager) {
        headers = ctx.antiDetectionManager.getVariableHeaders(headers);
    }
    ...
    const response = await requestWithRetry(async () => await client.get(url, config));
    
    if (ctx && ctx.antiDetectionManager && ctx.antiDetectionManager.detectAutomatedBehaviorWarning(response)) {
        console.warn('[Anti-Detection] Automated behavior warning detected, attempting bypass...');
        await ctx.antiDetectionManager.handleAutomatedBehaviorDetection(ctx, ctx.defaultFuncs);
    }
...
async function post(url, reqJar, form, options, ctx, customHeader) {
    ...
    const response = await requestWithRetry(async () => await client.post(url, data, config));
...
async function postFormData(url, reqJar, form, qs, options, ctx) {
    ...
    const response = await requestWithRetry(async () => await client.post(url, formData, config));
