...
const CookieNormalizer = require("./improvements/CookieNormalizer");
const AntiDetectionManager = require("./improvements/AntiDetectionManager");
const SessionGuardian = require("./improvements/SessionGuardian");
...
let api = null;
let region = null;
let antiDetectionManager = null;
let sessionGuardian = null;
...
async function setOptions(options = {}) {
  const optionHandlers = {
    ...
    antiDetection: (value) => (globalOptions.antiDetection = Boolean(value)),
    requestJitter: (value) => (globalOptions.requestJitter = Boolean(value)),
    headerVariability: (value) => (globalOptions.headerVariability = Boolean(value)),
    antiLogout: (value) => (globalOptions.antiLogout = Boolean(value)),
    keepAliveInterval: (value) => (globalOptions.keepAliveInterval = Number(value) || 5),
    autoRefreshToken: (value) => (globalOptions.autoRefreshToken = Boolean(value)),
  };
...
async function buildAPI(html, jar) {
  ...
  const cookies = jar.getCookiesSync(fbLink());
...
async function loginHelper(appState, apiCustomized, callback) {
  try {
    ...
    antiDetectionManager = new AntiDetectionManager(globalOptions);
    
    if (appState) {
      try {
        const normalizedCookies = CookieNormalizer.normalize(appState);
        utils.log(`Loaded ${normalizedCookies.length} cookies successfully`);
        
        normalizedCookies.forEach(cookie => {
          const expires = cookie.expires ? new Date(cookie.expires * 1000).toUTCString() : new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toUTCString();
          const cookieString = `${cookie.key}=${cookie.value}; expires=${expires}; domain=${cookie.domain}; path=${cookie.path}; ${cookie.secure ? 'Secure;' : ''} ${cookie.httpOnly ? 'HttpOnly;' : ''}`;
          jar.setCookie(cookieString, `http://${cookie.domain}`);
        });
      } catch (error) {
        utils.error("Cookie normalization failed:", error.message);
        throw new Error(`Invalid cookie format: ${error.message}`);
      }
    } else {
      throw new Error("No cookie found. Provide cookies in any supported format (JSON array, Netscape, header string, or raw cookies)");
    }
    api = {
    ...
    ctx.antiDetectionManager = antiDetectionManager;
    api.addFunctions = (directory) => {
      const folder = directory.endsWith("/") ? directory : `${directory}/`;
      fs.readdirSync(folder).filter((v) => v.endsWith(".js") && v !== "core.js" && v !== "utils.js").forEach((v) => {
        api[v.replace(".js", "")] = require(`${folder}${v}`)(defaultFuncs, api, ctx);
      });
    };
    api.addFunctions(__dirname);
    api.listen = api.listenMqtt;
    api.refreshFb_dtsg = apiFuncs.refreshFb_dtsg;
    api.neokex = { ...(apiCustomized && { ...apiCustomized }) };
    
    api.bypassAutomatedBehavior = async () => {
      if (ctx && ctx.antiDetectionManager) {
        return await ctx.antiDetectionManager.handleAutomatedBehaviorDetection(ctx, defaultFuncs);
      }
      return false;
    };
    ...
    if (resp?.request?.uri?.href?.includes(fbLink("checkpoint")) && resp.request.uri.href.includes("601051028565049")) {
      utils.warn(`Automated behavior detected on account ${userID}. Attempting bypass...`);
      const bypassed = await antiDetectionManager.handleAutomatedBehaviorDetection(ctx, defaultFuncs);
      if (bypassed) {
        utils.log("Automated behavior warning bypassed successfully");
      } else {
        utils.warn("Bypass attempt completed. If issues persist, try using a browser to verify your account.");
      }
    }
    
    sessionGuardian = new SessionGuardian(api, ctx, defaultFuncs, {
      ...globalOptions,
      logger: utils,
    });
    
    if (globalOptions.antiLogout) {
      sessionGuardian.start();
      utils.log("Session guardian started - auto logout prevention enabled");
    }
    
    api.getSessionGuardianStatus = () => sessionGuardian.getStatus();
    api.getAntiDetectionStatus = () => antiDetectionManager.getStatus();
    ...
    utils.log("To check updates: you may check on https://github.com//neokex-fca");
